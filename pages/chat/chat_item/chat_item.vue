		...
		<view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="textbox">
				...
				<view class="text-mode"  :class="isVoice?'hidden':''">
					<view class="box">
						<textarea auto-height="true" v-model="textMsg" @focus="textareaFocus" show-confirm-bar="false" cursor-spacing="12"/>
					</view>
...
// 						{type:"user",msg:{id:4,type:"voice",time:"13:05",userinfo:{uid:0,username:userinfos.username,face:userinfos.faceImg},content:{url:"/static/voice/2.mp3",length:"00:06"}}},
			...
			getMsgList(){
				...
				let list = [
					{type:"user",msg:{id:1,type:"text",time:"12:56",userinfo:{uid:0,username:userinfos.username,face:userinfos.faceImg},content:{text:"天王盖地虎"}}},
				// {type:"user",msg:{id:6,type:"img",time:"12:59",userinfo:{uid:1,username:"会话对象",face:"/static/face/face_2.jpg"},content:{url:"/static/img/q.jpg",w:1920,h:1080}}},
				]
				// 获取消息中的图片,并处理显示尺寸
				for(let i=0;i<list.length;i++){
					if(list[i].type=='user'&&list[i].msg.type=="img"){
			...
			sendText(){
				...
				console.log(msg);
				let content = this.replaceEmoji(msg.text);
			...
			sendMsg(content,type){
				...
				let lastid = this.msgList[this.msgList.length-1].msgId;
				lastid++;
				
				let message = {msgId:Math.floor(Math.random()*10000+1),userId:userinfos.userId,username:userinfos.nickName,face:userinfos.avatarUrl,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
				// 发送消息
